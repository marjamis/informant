...
def results(request):
    ...
    # load key for when a gzip/tarball of the CloudTrail logs are provided
        ...
        if general.searchKeyExists(searchKey):
            return render(request, 'public/error.html', {
                "message": "Key already exists. Select a unique key."
            })
    ...
    # search_key key for when a pull from the S3 for CloudTrail logs is
    # required, uses the passed on details to do this
        ...
        general.saveSearch(
            ...
            datetime.strptime(request.GET['date'], "%Y/%m/%d")
        )
        cloudtrailUtil.cloudtrail_pull(request.GET["bucket_region"],
                                       request.GET['bucket'],
                                       request.GET['prefix']+'AWSLogs/' +
                                       request.GET["accountId"] +
    ...
    # Uses the CloudTrail raw JSON data to generate graphs and tables
    # relating to the API calls in the data
    output = cloudtrailUtil.load_data(constants.SEARCH_DIR+'/'+searchKey)
    # A List of the subset of API calls listed in the Results page
    # Configured in a list for easy add or removing if required
    subsetList = ["eventTime", "eventName", "userAgent"]
    # A list of the graphs that will be generated from the data.
    # NOTE: If these are changed(mainly added/removed) make sure the required
    # changes on the results template are made for viewing
    graphList = ['eventName', 'eventSource', 'userAgent', 'awsRegion']
    # Using the CloudTrail JSON logs generates the required graphs and
    # dictionary of the subset of API calls
...
def full_results(request):
    try:
        return render(request, 'public/full_results.html', {
            'outputHeaders': constants.OUTPUT_HEADERS,
            'output': (
                cloudtrailUtil.load_data(constants.SEARCH_DIR+'/'+request.GET["prev"])
            )
        })
