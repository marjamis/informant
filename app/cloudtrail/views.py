...
from cloudtrail.models import Searches, Files
...
import boto, sys, shutil, gzip, os
...
def results(request):
  ...
  #search_key key for when a pull from the S3 for CloudTrail logs is required, uses the passed on details to do this
    ...
    general.saveSearch(request.GET["search_key"], request.GET["accountId"], datetime.strptime(request.GET['date'], "%Y/%m/%d"))
    cloudtrailUtil.cloudtrail_pull(request.GET["bucket_region"],
      request.GET['bucket'],
      request.GET['prefix']+'AWSLogs/'+request.GET["accountId"]+'/CloudTrail/us-west-2/'+ request.GET['date']+'/',
